...
public class Reconciliation {
    ...
    public Reconciliation(double[] _rawMeasurement, double[] _standardDeviation, double[][] _incidenceMatrix) {
        if (_rawMeasurement != null && _standardDeviation != null && _incidenceMatrix != null) {
            if (_rawMeasurement.length == _standardDeviation.length && _standardDeviation.length == _incidenceMatrix[0].length) {
                ...
                double[] aux_StandardDeviation = (double[])_standardDeviation.clone();
                ...
                for(int i = 0; i < aux_varianceMatrix.length; ++i) {
                    for(int j = 0; j < aux_varianceMatrix[0].length; ++j) {
                        if (i == j) {
                            ...
                            aux_varianceMatrix[i][j] = 0.0;
                        }
                    }
                }

                this.varianceMatrix = new SimpleMatrix(aux_varianceMatrix);
                SimpleMatrix aux1 = (SimpleMatrix)this.varianceMatrix.mult((SimpleMatrix)this.incidenceMatrix.transpose());
                aux1 = (SimpleMatrix)aux1.mult((SimpleMatrix)((SimpleMatrix)this.incidenceMatrix.mult(aux1)).invert());
                this.adjustment = (SimpleMatrix)aux1.mult((SimpleMatrix)this.incidenceMatrix.mult(this.rawMeasurement));
                this.reconciledFlow = (SimpleMatrix)this.rawMeasurement.minus(this.adjustment);
                DMatrixRMaj temp = (DMatrixRMaj)this.reconciledFlow.getMatrix();
        ...
        if (_rawMeasurement != null && _standardDeviation != null && _incidenceMatrix != null) {
            if (_rawMeasurement.length == _standardDeviation.length && _standardDeviation.length == _incidenceMatrix.length) {
                ...
                for(int i = 0; i < _rawMeasurement.length; ++i) {
                    double auxMP = Math.pow(_rawMeasurement[i] * _standardDeviation[i], 2.0);
                    auxDiagonalMatrix[i][i] = 2.0 / auxMP;
                    auxWeightsArray[i] = 2.0 * _rawMeasurement[i] / auxMP;
                }
                ...
                this.reconciledFlow = (SimpleMatrix)((SimpleMatrix)this.diagonalMatrix.invert()).mult(this.weightsArray);
                DMatrixRMaj temp = (DMatrixRMaj)this.reconciledFlow.getMatrix();
                this.reconciledFlowDouble = temp.getData();
            } else {
                System.out.println("the rawMeasurement and/or standardDeviation and/or incidenceMatrix have inconsistent data/size.");
